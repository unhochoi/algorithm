...
int dp(int n){
    ...
    for (int i=2; i<=n; ++i){
        for (int j=1; j<=3; ++j){
            ...
            if (j==1){
                 accum_cost[i][1] = cost[i][1] + min_2(accum_cost[i-1][2], accum_cost[i-1][3]);
            }
            ...
            // [i번째 초록집의 비용 + i-1번째 집까지의 최소 누적 비용]으로 누적 비용을 갱신
                ...
                accum_cost[i][2] = cost[i][2] + min_2(accum_cost[i-1][1], accum_cost[i-1][3]);
            }
            ...
            // [i번째 파란색의 비용 + i-1번째 집까지의 최소 누적 비용]으로 누적 비용을 갱신
                ...
                accum_cost[i][3] = cost[i][3] + min_2(accum_cost[i-1][1], accum_cost[i-1][2]);
            }
        }
    }
    ...
    return min_3(accum_cost[n][1], accum_cost[n][2], accum_cost[n][3]);
}
